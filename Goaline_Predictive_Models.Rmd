...
## For this scenario, I am pretending we have only completed 14 weeks of the 2022
## season.
goal_offense_wk_14 <- goal_offense %>% 
        filter(week <= 14)
...
goal_offense_wk_14 <- goal_offense_wk_14 %>% 
...
play_proportions <- goal_offense_wk_14 %>% 
...
train_obs <- sample(
        x = 1:nrow(goal_offense_wk_14),
        size = round(nrow(goal_offense_wk_14)*0.8)
)

## Create train/test data frames
train_dat <- goal_offense_wk_14[train_obs,]
test_dat <- goal_offense_wk_14[-train_obs,]
...
The AUC is `r round(pROC::auc(league_roc),2)`, which is an acceptable level of classification. Essentially, the model may do an acceptable job of correctly predicting the play call. An AUC < 0.7 is believe to be poor level of classification, however, the goal here is to _aid_ the defensive coordinator, therefore an acceptable level of prediction is likely still useful.
...
Each team's regression line for the relationship of line of scrimmage and play call probability differ depending on the down. Note: some teams either have complete separation (HOU 3rd down) or only called one type of play call (ARI 3rd down). This would be an issue if building a regression model for an individual team, however, that is not the route that will be taken.
...
The AUC for the multilevel model is `r round(pROC::auc(team_roc),2)`, so it is still performing at an acceptable level.
